...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-14 h-[calc(100vh-56px)] w-20 bg-[#0f0f0f] flex flex-col items-center pt-3 gap-0 z-40">
      {items.map(({ href, label, icon }) => {
        ...
        return (
          <Link
            key={label}
            href={href}
            className={`flex flex-col items-center justify-center gap-1.5 w-full py-4 rounded-xl transition ${
              active ? "bg-[#272727] text-white" : "text-[#aaa] hover:bg-[#1a1a1a] hover:text-white"
            }`}
          >
            {icon}
